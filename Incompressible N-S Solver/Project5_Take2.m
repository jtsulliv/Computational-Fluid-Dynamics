...
for i = 2:(Ny-1);
    for j = 2:(Nx-1);
        ...
        %GOING TO SWITCH INDICES AND USE THE SIMPLIFIED FORM OF (10.67)
        z(i,j) = zo(i,j) + dt * (...
            ((-uo(i,j)) * (( zo(i,j+1) - zo(i,j-1) ) / (2*dx)))...
            - ( vo(i,j)*((zo(i+1,j) - zo(i-1,j) ) / (2*dy) ))...
            + (1/Re) * ( ( (zo(i,j+1) - 2*zo(i,j) + zo(i,j-1))/(dx^2) ) + ( (zo(i+1,j) - 2*zo(i,j) + zo(i-1,j))/(dy^2) ) ) );
    end
end
...
for i = 2:(Ny-1);
    for j = 2:(Nx-1);
        phi_oo(i,j) = (1-w) * phi_o(i,j) + (w/(2*(1+b^2)))... 
            * (dx^2 * zo(i,j) + phi_o(i,j+1) + phi_oo(i,j-1) + b^2*(phi_o(i+1,j) + phi_oo(i-1,j)));
    end
end
